...
which **re-decode the descriptor pool on every call**. For the same ~1 MB message
**with** `DescriptorCache` (pool decoded once), see `benchmark_large_hot_loop.py`.

Why this matters for large messages
-----------------------------------
On a *small* message the per-call descriptor decode dominates, so the free
functions look much slower than the cached path (`benchmark.py` vs
`benchmark_hot_loop.py`). On a **~1 MB** message the **payload conversion itself**
dominates instead — so even without the cache, protoruf can still show a real gain
over pure Python. This benchmark measures exactly that: free functions, big
payload.
...
def main() -> None:
    print("Preparing the large-message benchmark (~1 MB, free functions)...\n")
    ...
    # 3. google.protobuf setup (optional). The factory/pool is built once; google
    #    has no equivalent of re-decoding its pool per call, so this is the honest
    #    "protoruf free functions vs google" comparison (mirrors benchmark.py).
    ...
    # 4. Comparison table.
    print("=" * 78)
    print(f"Large-message results — free functions (~{json_mb:.2f} MB, {ITERATIONS:,} messages)")
    print("=" * 78)
    header = f"{'Operation':<28} {'google.protobuf':<18} {'protoruf (free)':<18} {'Speedup':<8}"
    print(header)
    print("-" * len(header))
